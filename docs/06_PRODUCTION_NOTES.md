...
one continuous 11:20 cut, 2.39:1, 24 fps, every scene of the screenplay in order,
...
| | |
|---|---|
| **Picture** | Procedural location plates (`render/scenery.py`) painted once per location at 1.55× frame size, so camera moves are real crops through an oversized plate rather than post-hoc zooms. Layered building silhouettes, perspective roads, lit windows with true Gaussian bloom. |
